...
/// Helper to load Metal code from a resource bundle
@MainActor
final class MetalKernelLoader {
	static func loadLibrary(device: MTLDevice) throws -> MTLLibrary {
		// Try to load from the compiled library first (if part of the main app)
		do {
			return try device.makeDefaultLibrary(bundle: .main)
		} catch {
			// If that fails, use the embedded shader source
			let source = """
